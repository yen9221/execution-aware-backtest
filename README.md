...
This first partial-rebalance primitive deliberately uses zero-cost sizing. Representative target matching is validated only with zero fees and zero slippage. Fee-aware sizing, slippage-aware sizing, post-cost exact target solving, rebalance tolerance, and minimum trade notional are not implemented. The engine does not consume fractional target weights yet, so this is not a complete continuous-weight backtest path. Existing binary `TargetPosition` conversion and engine behavior remain unchanged, and fractional weights are never silently coerced into that binary path.
