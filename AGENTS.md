...
- The current execution and prediction-artifact workflow is strictly hourly. Existing daily predictions are not directly compatible and must not be converted to hourly by forward filling, duplication across hourly rows, interpolation, nearest-timestamp matching, timestamp shifting, timestamp relabeling, or implicit overlap selection. Real integration requires a genuine frozen hourly prediction artifact; any future interval generalization must be explicit and must neither weaken nor silently alter the strict hourly workflow.
...
9. **ML integration compatibility decision and deferred integration.** Record whether an available frozen prediction artifact satisfies the current hourly timestamp, interval, provenance, and exact-alignment contracts. Existing daily BTC/ETH outputs must not be resampled or transformed into hourly predictions. Real ML execution integration remains an optional future extension only after the separate ML repository produces a genuine frozen hourly artifact. Do not run a model or backtest as part of the compatibility decision.
...
**Optional future extension — frozen hourly ML integration.** After a genuine compatible hourly artifact exists and the policy specification is complete, integrate it as a separate diagnostic without overwriting the standalone rule-based results.
